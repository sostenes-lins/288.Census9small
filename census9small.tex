...
{\em ``The ultimate goal of the theory would be in providing solutions to:
 ...
 independently and simpler that that of quantum groups.  In the early 
 ...
 some are  extremely active in this era Pós-Perelman, \cite{friedl3}. 
 ...
 graphs. We mean to provide a strategy for a segmented answer of Hempel's questions. 
 ...
 good examples (hard to find) are important in obtaining progress in a general theory.
 The classifying $INV$ for the 9-small 3-manifolds is
...
\end{figure} 
 ...
 simple way an specific 3-gem inducing the 3-manifold. If we consider these three
 ...
 straighforward to obtain a canonical gem from a blink. Proving that two gems induce the same
 ...
 (at a computational level) whether two 3-manifolds are or are not homeomorphic. And census
 become available!
...
\cite{kauffman1991knots}. I got influenced by Kauffman's insistence
 in considering blackboard framed links \cite{kauffman1991knots} to use the plane to remove the 
 number attached to the components, thus obtaining, esssentially, the blinks. 
 No doubt that blinks are simple mathematical objects. 
 Nevertheless, as a consequence of Martelli's reformulation of Kirby's calculus as complemented by
 Fenn-Rourke and Kauffman, I will prove shortly that they hold, in their gist, the mystery of 3-manifolds.
 
\begin{figure}[!h]
\begin{center}
\includegraphics[width=8cm]{A.figs/FennRourkeMove.pdf} 
\caption{\sf The Fenn-Rourke single $n$-parametrized 
move: a topological blow-down in the number of components. 
The number $n\ge0$ of vertical
strands crossing the unknot is arbitrary (here $n=3$).
The box marked with $+1$ (-1) indicates one full counterclockwise (clockwise) twist.}
...
\caption{\sf The two wo doubts left in L. Lins's thesis are solved showing that BLINK alone provides
the topological classification of the 9-small 3-manifolds. In both cases the manifolds induced
by the shaded blink-link pairs are not homeomorphic to the others in the class. These are already
known to be homeomorphic by BLINK.}
\label{fig:nine126}
\end{center}
\end{figure} 
...
up to $n=9$. Conceptually we could go on forever, finding in the way tougher and tougher examples to be distinguished
by yet to be found new invariants.
 ...
 after we put a Challenge in the arXiv, \cite{linslins2013A} these doubts were resolved by
 ...
 conjectures $k=1$. A recent finding by C. Hodgson concerning manifolds $T[71]$ and $T[79]$
 in the notation of \cite{lins2007blink} shows that the 3 invariants are not enough to
 decide the pair. This pair is the first one of 11 pairs that we treat in some tougher challenges
 ...
 to be non-homeomorphic by more sofisticated ideas. N. Dunfield also showed that $T[71]$ and $T[79]$
