...
\begin{center}
{\em $UP3M=\{$ gem (Italian School of crystallizations \&S. Lins), 
triangulation (E. Moise), 
Hegaard diagram, 
special spine (Matveev), 
homology based framed link (SnapPy),
$\pm \infty$-fractional framed link (D. Rolfsen), 
integer framed, 
link  (Lickorish \& Kirby),
blackboard framed link (L. Kauffman), 
blink (S. Lins) $\}.$}
\end{center}
...
\begin{figure}[H]
   \begin{center}
      \leavevmode
      \includegraphics[width=16.0cm]{A.figs/4pipeline.pdf}
   \end{center}
   \vspace{-0.7cm}
   ...
   BLINK has in its data basis the sets $U_9$ and $U_{10}$ with respectively
...
$HG12QI$-classes of blink in $U_9$.
}
...
census generating algorithms are fully parallellizable, and this parallelization was not used.
 ...
 as looking for small index coverings which depend on the infinite 
...
does not take so long to execute 
while having no guaranty of success.
}
